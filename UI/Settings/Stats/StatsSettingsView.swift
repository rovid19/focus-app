    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 24) {
                ...
                StatsListSection(
                    controller: controller.homeController?.statsController ?? StatsController(),
        ...
        .onChange(of: statsManager.stats) { _ in
            Task { @MainActor in animateElements() }
        }
    }

    // MARK: - StatsManager accessor

    private var statsManager: StatisticsManager { StatisticsManager.shared }
...
// MARK: - Stats Content Section

private struct StatsContentSection: View {
    let statsManager: StatisticsManager
    let statsListVisible: Bool
    let emptyViewVisible: Bool
        ...
        VStack(spacing: 12) {
            if statsManager.isLoading {
                ...
                if statsManager.stats.isEmpty {
                    ...
                    StatsList(statsManager: statsManager, statsListVisible: statsListVisible)
                }
            }
        }
    }
